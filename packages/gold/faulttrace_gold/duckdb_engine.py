...
class DuckDBEvaluator:
    ...
    def _evaluate_with_con(
        self, query: QuerySpec, con: duckdb.DuckDBPyConnection
    ) -> dict[str, Any]:
    ...
    def _proportion(
        self, spec: ProportionSpec, scope_sql: str, con: duckdb.DuckDBPyConnection
    ) -> tuple:
    ...
    def _comparison(
        self, spec: ComparisonSpec, scope_sql: str, con: duckdb.DuckDBPyConnection
    ) -> tuple:
    ...
    def _group_sql_measure(
        self, spec: ComparisonSpec, where_clause: str, con: duckdb.DuckDBPyConnection
    ) -> float | None:
    ...
    def _top_k(self, spec: TopKSpec, scope_sql: str, con: duckdb.DuckDBPyConnection) -> tuple:
        ...
        result = [{spec.group_by_field: row[0], "value": float(row[1])} for row in rows]
        ids_sql = f"SELECT record_id FROM records WHERE {scope_sql}"
        ids = [row[0] for row in con.execute(ids_sql).fetchall()]
