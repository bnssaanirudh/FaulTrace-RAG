...
class PandasEvaluator:
    ...
    def _count(
        self, spec: CountSpec, df: pd.DataFrame, ids: list[str]
    ...
    def _sum(
        self, spec: SumSpec, df: pd.DataFrame, ids: list[str]
    ...
    def _mean(
        self, spec: MeanSpec, df: pd.DataFrame, ids: list[str]
    ...
    def _comparison(
        ...
        return (
            result,
            list(df_a["record_id"].tolist() if "record_id" in df_a.columns else [])
    ...
    def _group_measure(self, spec: ComparisonSpec, df: pd.DataFrame) -> float | None:
        if len(df) == 0:
            return None
    ...
    def _top_k(
        self, spec: TopKSpec, df: pd.DataFrame, ids: list[str]
        ...
        if spec.tie_policy == TiePolicy.FIRST:
            ...
            if k_val < len(grouped):
                ...
                top = (
                    grouped[grouped["_measure"] >= kth_val]
                    if not spec.ascending
                    else grouped[grouped["_measure"] <= kth_val]
                )
            else:
                top = grouped
        else:
    ...
    def _trend(
        self, spec: TrendSpec, df: pd.DataFrame, ids: list[str]
    ) -> tuple[list[dict], list[str], dict]:
