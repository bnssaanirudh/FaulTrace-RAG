...
from faulttrace_core.retrieval import DocumentRenderer, RetrievalUnit
...
class SentenceTransformerProvider(EmbeddingProvider):
    ...
    def __init__(self, model_name: str = "all-MiniLM-L6-v2"):
        try:
            ...
            raise ImportError("sentence-transformers is required for this provider") from e
...
class DenseRetriever:
    ...
    def _load_and_index(self):
        ...
        for _, row in df.iterrows():
            ...
            for unit in units:
                ...
                meta_prefix = (
                    f"{unit.metadata.get('title', '')} {unit.metadata.get('category', '')}: "
                )
        ...
        if all_embeddings:
            ...
            logger.info(
                f"Built FAISS index for {self.world_id} with {len(self.units)} units (dim={self.provider.dimension})"
            )
    ...
    def retrieve(
        ...
        for score, idx in zip(scores[0], indices[0], strict=False):
            ...
            results.append({"unit": unit, "score": float(score), "rank": rank})
            rank += 1
