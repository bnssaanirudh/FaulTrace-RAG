...
class LossDiagnostic(BaseModel):
    ...
    raw_error: float | None = None  # Absolute difference for continuous values
    jaccard_distance: float | None = None  # For sets/TopK
    status: str = "valid"  # "valid", "invalid", "abstained", "partial"


def compute_loss(
    pipeline_answer: Any, gold_answer: Any, agg_spec: AggregationSpec, tolerance: float = 1e-4
) -> LossDiagnostic:
...
def _compute_topk_loss(p_ans: Any, g_ans: Any) -> LossDiagnostic:
    ...
    return LossDiagnostic(
        normalized_loss=jaccard_dist, jaccard_distance=jaccard_dist, status="valid"
    )
...
def _compute_trend_loss(p_ans: Any, g_ans: Any) -> LossDiagnostic:
    ...
    dict_g = {
        str(x.get("bucket", "")): float(x.get("value", 0)) for x in g_ans if isinstance(x, dict)
    }
