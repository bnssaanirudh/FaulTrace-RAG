...
class P5CertifiedMERPipeline(P4FullScopeMERPipeline):
    ...
    def _execute(
        ...
        schema = SchemaGenerator.generate_extraction_schema(
            query.fact_spec, query.aggregation_spec, scope_res.eligible_record_ids
        )
        ...
        for unit in plan.units:
            ...
            def run_attempt(
                attempt_num: int, reason: RepairReason = RepairReason.NONE, error_ctx: str = ""
            ) -> tuple[bool, dict | None]:
                nonlocal token_in, token_out
                cache_key = self.cache.generate_key(
                    self.provider_id, "default", prompt_hash, records_hash, schema_hash, attempt_num
                )
            ...
            if success and parsed:
                ...
                # Attempt 1 (Repair)
                success, parsed = run_attempt(
                    1, RepairReason.INVALID_JSON, "Failed to parse valid extraction array"
                )
                repaired_count += 1
                if success and parsed:
                    extracted_rows.extend(parsed.get("extracted_records", []))
                    ...
                    # Attempt 2 (Hard cap)
                    success, parsed = run_attempt(
                        2, RepairReason.INVALID_JSON, "Second failure. Hard cap repair."
                    )
                    repaired_count += 1
                    if success and parsed:
                        extracted_rows.extend(parsed.get("extracted_records", []))
        ...
        events.append(
            self._make_event(
                run_id, "aggregate", TraceEventType.AGGREGATE, f"Answer: {answer_value}"
            )
        )
