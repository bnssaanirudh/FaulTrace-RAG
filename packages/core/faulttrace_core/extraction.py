...
class ExtractedField(BaseModel):
    ...
    snippet_evidence: str | None = Field(
        None, description="Exact text snippet from the record proving this value"
    )
    is_missing: bool = Field(
        default=False, description="True if the value could not be found in the text"
    )
...
class RecordExtraction(BaseModel):
    ...
    is_relevant: bool = Field(
        ..., description="True if the record contains information relevant to the query"
    )
    extracted_fields: list[ExtractedField] = Field(
        default_factory=list, description="Fields extracted from this record"
    )
    reasoning: str | None = Field(
        None, description="Brief explanation of inclusion/exclusion and extraction"
    )


class QueryExtractionResult(BaseModel):
    query_id: str = Field(..., description="The ID of the query")
    record_extractions: list[RecordExtraction] = Field(
        default_factory=list, description="Extractions per record evaluated"
    )
