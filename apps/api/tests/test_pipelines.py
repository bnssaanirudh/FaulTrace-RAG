...
def sample_df() -> pd.DataFrame:
    ...
    categories = rng.choice(
        ["Electronics", "Books", "Sports & Outdoors", "Clothing", "Beauty"],
        size=n,
    ...
    return pd.DataFrame(
        {
            ...
            "source_record_id": [f"src{i:04d}" for i in range(n)],
            ...
            "raw_payload_hash": ["hash" * 8 for _ in range(n)],
        }
    )
...
def parquet_path(sample_df, tmp_path) -> Path:
    p = tmp_path / "data" / "worlds" / "test_world" / "records.parquet"
...
def test_p1_scope_fault_logged(sample_df, count_query, gold_count, tmp_path):
    ...
    assert scope_events
    assert any("P1 FAULT" in ev.message or "wrong" in ev.message.lower() for ev in scope_events)
...
def test_p2_fact_fault_logged(sample_df, mean_query, gold_mean, tmp_path):
    ...
    assert fact_events
    assert any("FAULT" in ev.message or "noise" in ev.message.lower() for ev in fact_events)
...
def test_attribution_shapley_sum_near_1(sample_df, count_query, gold_count, tmp_path):
    ...
    assert (
        abs(phi_sum + result.interaction_term - result.total_error) < 0.01
    )  # should sum to total error
...
# ── Research pipelines (P1-P3 Direct/Extract) ─────────────────────────────────


def test_p1_direct_bm25_runs(sample_df, count_query, gold_count, tmp_path, parquet_path):
    from faulttrace_pipelines.p1_direct_bm25 import P1DirectBM25Pipeline

    p1 = P1DirectBM25Pipeline(artifacts_dir=tmp_path / "p1_bm25")
    run, events, _ = p1.run(
...
def test_p2_direct_dense_runs(sample_df, count_query, gold_count, tmp_path, parquet_path):
    ...
    p2 = P2DirectDensePipeline(artifacts_dir=tmp_path / "p2_dense")
    run, events, _ = p2.run(
        query=count_query, df=sample_df, parquet_path=parquet_path, gold_answer=gold_count
    )
    assert run.status.value == "completed"
...
def test_p3_extract_aggregate_runs(sample_df, count_query, gold_count, tmp_path, parquet_path):
    from faulttrace_pipelines.p3_extract_aggregate import P3ExtractAggregatePipeline

    p3 = P3ExtractAggregatePipeline(artifacts_dir=tmp_path / "p3_ea")
    run, events, _ = p3.run(
        query=count_query, df=sample_df, parquet_path=parquet_path, gold_answer=gold_count
    )
    assert run.status.value == "completed"
