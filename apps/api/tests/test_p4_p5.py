...
def dummy_query():
    spec_json = {
        ...
        "scope_predicate": {"kind": "eq", "field": "category", "value": "Electronics"},
        "fact_spec": {"fields": ["rating"], "derived_fields": [], "null_policy": "exclude"},
        "aggregation_spec": {"kind": "mean", "field": "rating"},
        "world_id": "world-test",
        "template_id": "manual",
    }
...
def test_p4_full_scope_mer(dummy_query, dummy_df, tmp_path):
    ...
    ans, events, components, tin, tout = pipeline._execute(
        "test-run", dummy_query, dummy_df, None, None
    )
...
def test_p5_certified_mer(dummy_query, dummy_df, tmp_path):
    ...
    ans, events, components, tin, tout = pipeline._execute(
        "test-run-p5", dummy_query, dummy_df, None, None
    )
