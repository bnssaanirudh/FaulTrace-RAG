...
from faulttrace_api.database import QueryRow, RunRow, TraceEventRow, get_db
from faulttrace_api.models import (
    CreateRunRequest,
    PaginatedResponse,
    RunResponse,
    TraceEventResponse,
...
async def create_run(
    ...
    run_row = RunRow(
        ...
        raw_answer=str(run.raw_answer) if run.raw_answer is not None else None,
        policy_decision=run.policy_decision,
        final_presented_answer=str(run.final_presented_answer)
        if run.final_presented_answer is not None
        else None,
...
async def create_map_plan(
    ...
    if not scope_res.is_success:
        raise HTTPException(
            status_code=400, detail=f"Scope enumeration failed: {scope_res.failure_code}"
        )
...
async def get_run_attribution(run_id: str, db: Session = Depends(get_db)):
    ...
    parent_run = PipelineRun(
        ...
        artifact_references=json.loads(run_row.artifact_refs_json)
        if run_row.artifact_refs_json
        else {},
    )
...
async def batch_attribution(run_ids: list[str], db: Session = Depends(get_db)):
    """
