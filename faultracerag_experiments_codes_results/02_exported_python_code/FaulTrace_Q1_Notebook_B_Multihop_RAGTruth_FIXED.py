"""Exported code cells from FaulTrace_Q1_Notebook_B_Multihop_RAGTruth_FIXED.ipynb.
...
def normalize_2wiki(ex):
    """Normalize both common 2WikiMultihopQA Hugging Face schemas."""
    m = ex.get("metadata", ex)
    ctx = m["context"]
    sf = m["supporting_facts"]

    # cmriat/2wikimultihopqa uses `content`.
    # Hotpot-compatible mirrors commonly use `sentences`.
    sentence_lists = ctx.get("content")
    if sentence_lists is None:
        sentence_lists = ctx.get("sentences")
    if sentence_lists is None:
        raise KeyError(
            f"Unsupported 2Wiki context schema. Available keys: {list(ctx.keys())}"
        )

    titles = ctx.get("title")
    if titles is None:
        raise KeyError(
            f"2Wiki context is missing `title`. Available keys: {list(ctx.keys())}"
        )

    docs = {
        str(t): [str(s) for s in ss]
        for t, ss in zip(titles, sentence_lists)
    }
...
# ============================================================
# Regression test for both supported 2Wiki schemas.
_cmriat_example = {
    "id": "dev_test",
    "question": "test?",
...
_hotpot_style_example = {
    "id": "dev_test_2",
    "question": "test?",
    "answer": "answer",
    "supporting_facts": {"title": ["Doc B"], "sent_id": [1]},
    "context": {
        "title": ["Doc B"],
        "sentences": [["S0.", "S1."]]
    },
...
_a = normalize_2wiki(_cmriat_example)
_b = normalize_2wiki(_hotpot_style_example)

assert _a["docs"]["Doc A"][0] == "Sentence one."
assert ("Doc A", 0) in _a["supports"]
assert _b["docs"]["Doc B"][1] == "S1."
assert ("Doc B", 1) in _b["supports"]

print("2Wiki schema regression tests passed.")

# ============================================================
# NOTEBOOK CELL 13 / CODE CELL 8
# ============================================================
